...
import matplotlib.pyplot as plt  # ติดตั้งหากยังไม่มี: pip install matplotlib
...
def run_for_cores(n, cores):
    ...
    cmd = [
        ...
        '-n', str(cores),
        sys.executable, '-m', 'lib.mpi_entry', str(n)
    ]
    proc = subprocess.run(cmd, capture_output=True, text=True, cwd=CODE_DIR, timeout=300)
...
def main():
    ...
    for num_cores in range(1, 5):
        ...
        if rc == 0:
            ...
            print(f"{num_cores} core(s): {factors:30s} | compute_time={compute_time if compute_time is not None else 'N/A':>8} s | wall={elapsed_wall:.4f}s")
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["cores", "factors", "compute_time", "returncode", "stderr", "wall_time"])
        for row in results:
            w.writerow(row)
